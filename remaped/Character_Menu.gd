...
func update_buttons():
	...
	for index in range(IMPLANTS.size()):
		if Global.get_node("Multiplayer").is_implant_banned(IMPLANTS[index].i_name):
			EQUIPMENT_BUTTONS[index].modulate = Color(1, 0.2, 0.2)
...
func _on_mouse_entered(i):
	if i >= 0 and i < IMPLANTS.size() and Global.get_node("Multiplayer").is_implant_banned(IMPLANTS[i].i_name):
