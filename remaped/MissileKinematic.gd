		...
		if collision:
			if not piercing:
				...
				for i in range(4):
					...
					NetworkBridge.inherit_damage_source(new_shrapnel, self)
				...
				if collision.collider.has_method("piercing_damage"):
					NetworkBridge.apply_damage(self, collision.collider, "piercing_damage", [150, (global_transform.origin - collision.position).normalized(), global_transform.origin, global_transform.origin])
				if collision.collider.has_method("damage"):
					NetworkBridge.apply_damage(self, collision.collider, "damage", [150, (global_transform.origin - collision.position).normalized(), global_transform.origin, global_transform.origin])
				else :
